...
def get_db_connection():
    return psycopg2.connect(
        ...
        database=os.getenv("POSTGRES_DB", "youtube_chat"),
        ...
        with conn.cursor() as cur:
            ...
            cur.execute("""
                CREATE TABLE conversations (
                    id TEXT PRIMARY KEY,
                    video_id TEXT REFERENCES transcript(id),
...
def save_conversation(conversation_id, video_id,question, answer_data, timestamp=None):
        ...
        with conn.cursor() as cur:
            cur.execute(
                ...
                (
                    ...
                    timestamp  # Ensure this is correctly assigned or passed
                )
            )
        conn.commit()
    finally:
...
def save_feedback(conversation_id, video_id, feedback, timestamp=None):
    if timestamp is None:
        timestamp = datetime.now(tz)
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                "INSERT INTO feedback (conversation_id, video_id,feedback, timestamp) VALUES (%s, %s, %s, COALESCE(%s, CURRENT_TIMESTAMP))",
                (conversation_id, video_id, feedback, timestamp),
            )
        conn.commit()
    finally:
        conn.close()

def save_transcript(video_id, language_code, language, timestamp=None):
    if timestamp is None:
        timestamp = datetime.now(tz)
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                "INSERT INTO transcript (id, language_code, language) VALUES (%s, %s, %s)",
                (video_id, language_code, language),
            )
        conn.commit()
    finally:
        conn.close()
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute("""
                ...
                RETURNING timestamp;
            """, 
            ('test', 'test video id', 'test question', 'test answer', 'test model', 0.0, 0.0, 
